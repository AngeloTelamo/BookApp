...
namespace ASI.Basecode.WebApp.Controllers
{
    public class AdminController : ControllerBase<AdminController>
    {
        ...
        {
            var model = _bookMasterService.GetBooks(bookId); // refers in BookmasterService
            return View(model);
        }
        ...
        public IActionResult AdminBookUpdate(BookMasterEditViewModel model)
        {
            ...
            // If there's an error or if the model is not valid, return to the update page with the current data
            return View("AdminBookUpdate", model);
        }


        [HttpGet]
        public IActionResult RemoveBook(int bookId) //reference object sa BId kung naa ba sa repository 
        {
            try
            {
                _bookMasterService.DeleteBook(bookId); // refers in BookmasterService
                return RedirectToAction("AdminBookList");
            }
            catch (InvalidDataException ex)
            {
